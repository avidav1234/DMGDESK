...
class TabMacchina:
    ...
    def _create_ui(self):
        ...
        # Separator
        separator = ctk.CTkFrame(toolbar, width=2, fg_color=COLOR_BORDER, height=35)
        separator.pack(side="left", padx=15, fill="y")
        
        # Gruppo GENERA MAIN + Impostazioni
        main_group = ctk.CTkFrame(toolbar, fg_color="transparent")
        main_group.pack(side="left", padx=5)
    ...
    def _genera_main(self):
        """Genera programma MAIN O9999 con CALIBRA ONLY configurabile."""
        if self.main.df.empty:
            messagebox.showwarning("Attenzione", "Nessun utensile nel database")
            return
        ...
        if success:
            ...
            if msg not in ["Annullato", "Generazione annullata"]:
