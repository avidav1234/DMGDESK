...
// Persistenza stati pallet via /api/pallet (pallet_state.json sulla share)
...
const STATI_MANUALI = ['vuoto', 'grezzo', 'guasto']
const STATI_LABEL   = {
  vuoto: 'Vuoto', grezzo: 'Grezzo',
  in_lavorazione: 'In lavorazione', finito: 'Finito', guasto: 'Guasto',
}
...
function PalCard({ p, selected, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      background: cfg.bg, border: `1.5px solid ${cfg.bd}`, borderRadius: 8,
      cursor: 'pointer', display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center', gap: 6,
      padding: 12, minHeight: 90,
      outline: selected ? '2px solid #7eb8f5' : 'none', outlineOffset: 2,
      ...
      <span style={{ fontSize: 10, fontWeight: 600, color: cfg.fg, fontFamily: 'var(--font-mono)', letterSpacing: '.06em', opacity: p.stato === 'vuoto' ? .7 : 1 }}>
        {cfg.label}
      </span>
      {p.commessa && (
        <span style={{ fontSize: 9, color: cfg.fg, opacity: .65 }}>{p.commessa}</span>
      )}
      {p.programma && (
        <span style={{ fontSize: 8, color: cfg.fg, opacity: .55, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textAlign: 'center' }}>
...
function StatoPicker({ palletNum, statoAttuale, onScegli, onClose }) {
  return (
    <div style={{
      ...
      {STATI_MANUALI.filter(s => s !== statoAttuale).map(s => {
        const c = PALLET_CFG[s]
        return (
          <button key={s} onClick={() => onScegli(s)} style={{
            display: 'block', width: '100%', textAlign: 'left',
            padding: '7px 10px', border: `1px solid ${c.bd}`, borderRadius: 5,
      ...
      <button onClick={onClose} style={{
        display: 'block', width: '100%', textAlign: 'center', padding: '5px',
        border: '1px solid #e2e8f0', borderRadius: 5, cursor: 'pointer',
        fontSize: 11, color: '#94a3b8', background: 'none', marginTop: 2,
      }}>
        Annulla
      </button>
    </div>
...
export default function CodaLavorazione() {
  const [pallet, setPallet]       = useState(
    Array.from({ length: 6 }, (_, i) => ({ numero: i + 1, stato: 'vuoto', programma: null, commessa: null }))
  )
  ...
  const loadAll = async (silent = false) => {
    try {
      ...
      // 1. Carica stati pallet dalla share (persistenti)
      try {
        const ps = await api.getPallet()
        if (ps?.pallet) setPallet(ps.pallet)
      } catch {}

      // 2. Carica stato live dalla macchina (log)
      try {
        const s = await api.getStatoMacchina()
        setStato(s)

        // 3. Se la macchina dice quale pallet è in lavorazione, sincronizza
        if (s.connessa && s.pallet_attivo != null) {
          await api.syncLavorazione({
            pallet_attivo: s.pallet_attivo,
            programma_attivo: s.programma_attivo,
          }).catch(() => {})
          // Ricarica stati aggiornati
          const ps2 = await api.getPallet()
          if (ps2?.pallet) setPallet(ps2.pallet)
        }
      } catch {}
  ...
  useEffect(() => {
    loadAll()
    timer.current = setInterval(() => loadAll(true), REFRESH_MS)
    return () => clearInterval(timer.current)
  }, [])

  const handleStato = async (num, nuovoStato) => {
    // Aggiorna subito UI (ottimistico)
    setPallet(prev => prev.map(p => p.numero === num ? { ...p, stato: nuovoStato } : p))
    setSelected(null)
    // Persisti sulla share
    try {
      await api.setStatoPallet(num, { stato: nuovoStato })
    } catch (e) {
      setError(`Errore salvataggio stato: ${e.message}`)
      loadAll(true) // ripristina
    }
  }
  ...
  return (
    <div className="fade-in" style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button className="btn btn-ghost" style={{ fontSize: 12 }} onClick={() => loadAll()}>↻ Aggiorna</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {[
          ...
          <div key={i} style={{
            ...
            borderLeft: `3px solid ${s.color}`, borderRadius: '0 6px 6px 0', padding: '8px 14px',
          }}>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 260px', gap: 12, minHeight: 0 }}>
        ...
        <div className="card" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10, overflow: 'auto' }}>
          ...
          <div style={{ fontSize: 10, color: '#94a3b8', borderTop: '1px solid #e2e8f0', paddingTop: 8, marginTop: 'auto' }}>
            Clicca su un pallet per cambiarne lo stato. IN LAVORAZIONE viene aggiornato automaticamente dalla macchina.
          </div>
        </div>
          ...
          <div className="card" style={{ padding: 14, flex: 1 }}>
            ...
            {stato?.connessa ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 20, fontWeight: 700, color: '#0d2d5e' }}>
