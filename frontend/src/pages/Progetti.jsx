...
function ProgramRow({pgm,gruppo,onStato,onOperatore,onTempo,onRemove,toolStatus,selected,onSelect}){
  ...
  return(
    <div style={{borderBottom:`1px solid ${T.border}`,borderLeft:isIpmInline?'3px solid #8B2FC9':'3px solid transparent',background:selected?'#EFF6FF':pgm.stato==='completato'?'#f0fdf4':['in_main','in_lavorazione','in_macchina'].includes(pgm.stato)?'#eff6ff':isIpmInline?'#F3E8FF':T.surface,opacity:pgm.stato==='completato'&&!selected?0.75:1,transition:'background 0.15s'}}>
              ...
              {previsioneVita.map((a,i)=>(
                <div key={i} style={{background:'#fff',border:'1px solid #ffcc80',borderRadius:8,
                  ...
                  <div style={{background:'#fff3e0',borderRadius:6,padding:'6px 10px',fontSize:11}}>
                    ...
                    <span style={{color:'#9a3412'}}> pgm {a.critico.numPgm} — {a.vitaRim}min disponibili / {a.consumoTot}min richiesti</span>
                  </div>
                </div>
              ))}
            </div>
